...
class PDFCraftApp:
    ...
    def _build_ui(self):
        ...
        notebook.add(split_tab, text="Разделить PDF")

        ttk.Label(split_tab, text="PDF-файл:").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        ttk.Button(out_frame, text="Обзор...", command=self._browse_output).grid(row=0, column=1)

        ttk.Label(split_tab, text="Режим разделения:").grid(row=4, column=0, sticky=tk.W, pady=(10, 5))
        mode_frame = ttk.Frame(split_tab)
        mode_frame.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))

        ttk.Radiobutton(
            mode_frame, text="Каждая страница → отдельынй PDF-файл",
            variable=self.split_mode, value="each_page"
        ).pack(anchor=tk.W)
        ttk.Radiobutton(
            mode_frame, text="Каждые N страниц → отдельный PDF-файл",
            variable=self.split_mode, value="every_n"
        ).pack(anchor=tk.W)

        pages_frame = ttk.Frame(mode_frame)
        pages_frame.pack(anchor=tk.W, padx=(20, 0))
        ttk.Label(pages_frame, text="Страницы в файле:").pack(side=tk.LEFT, padx=(0, 5))
        ttk.Entry(pages_frame, textvariable=self.pages_per_file, width=6).pack(side=tk.LEFT)

        ttk.Radiobutton(
            mode_frame, text="Отделить только опредленные страницы (например 1, 2, 5)",
            variable=self.split_mode, value="at_pages"
        ).pack(anchor=tk.W)
        split_frame = ttk.Frame(mode_frame)
        split_frame.pack(anchor=tk.W, padx=(20, 0))
        ttk.Label(split_frame, text="Номера страниц:").pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Button(split_tab, text="Разделить PDF-файл", command=self._split_pdf).grid(
            row=7, column=0, columnspan=2, pady=(10, 0)
        )

        # === Join tab ===
        join_tab = ttk.Frame(notebook, padding=15)
        notebook.add(join_tab, text="Объединить PDF-файлы")

        ttk.Label(join_tab, text="Список PDF-файлов (выставить в нужном порядке):").grid(row=0, column=0, sticky=tk.W, pady=(0, 5))
        ...
        ttk.Button(btn_frame, text="Дабавить PDF...", command=self._join_add_files).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Удалить выбранное", command=self._join_remove).pack(side=tk.LEFT, padx=(0, 5))
        ttk.Button(btn_frame, text="Очистить все", command=self._join_clear).pack(side=tk.LEFT, padx=(0, 5))
        ...
        ttk.Button(out_file_frame, text="Сорхранить в...", command=self._join_browse_output).grid(row=0, column=1)
        ...
        ttk.Button(join_tab, text="Объединить PDF", command=self._join_pdfs).grid(
        ...
        ttk.Button(comp_out_frame, text="Сохранить в...", command=self._compress_browse_output).grid(row=0, column=1)

        ttk.Label(compress_tab, text="Опции сжатия:").grid(row=4, column=0, sticky=tk.W, pady=(10, 5))
        opt_frame = ttk.Frame(compress_tab)
        opt_frame.grid(row=5, column=0, columnspan=2, sticky=tk.W, pady=(0, 10))
        ttk.Checkbutton(
            opt_frame, text="Сжатие потока контента без потерь",
            variable=self.compress_streams
        ).pack(anchor=tk.W)
        ttk.Checkbutton(
            opt_frame, text="Удалить дубликаты и неиспользуемые объекты",
